...
try:
    ...
    from app.db import (SessionLocal, init_db, Project, Recording, RecordingStep, Variable, Run, resolve_variables)
...
def list_variables(project_id: str):
    db = SessionLocal()
    try:
        vars = db.query(Variable).filter(Variable.project_id == project_id).all()
...
def export_jenkins(rid: str):
    db = SessionLocal()
    r = db.get(Recording, rid); db.close()
    feature = f"Feature: {r.name}\n  Scenario: Automated UI flow\n    Given I launch '{r.start_url}'\n"
    for s in r.steps: feature += f"    When I perform {s.action} on {s.label}\n"
...
def github_sync():
    ...
    buf.seek(0); return Response(buf.read(), media_type="application/zip")

@app.post("/api/runs")
def start_run(body: dict):
    db = SessionLocal()
    try:
        run = Run(target_id=body['target_id'], status="queued")
        db.add(run); db.commit(); db.refresh(run); rid = run.id
        async def bg():
        ...
        asyncio.create_task(bg()); return {"run_id": rid}
    finally: db.close()
...
def list_runs():
    ...
    try:
        ...
        for r in runs:
            ...
            baseline = [{"action":s.action, "label":s.label} for s in rec.steps] if rec else []
            res.append({"id":r.id, "status":r.status, "current_agent":r.current_agent, "investigation":r.investigation_log, "log":r.log, "baseline": baseline})
        return res
    finally: db.close()
...
async def ws_record(ws: WebSocket, rid: str):
    ...
    session = RecorderSession(rid, rec.start_url, len(rec.steps), on_f, on_e)
    await session.start()
    try:
        while True:
...
async def ws_run(ws: WebSocket, run_id: str):
    await ws.accept()
    q = asyncio.Queue(); RUN_SUBS.setdefault(run_id, []).append(q)
    try:
        while True:
