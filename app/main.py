...
app.mount("/", StaticFiles(directory="app/static", html=True), name="static")         "shared": r.shared, "status": r.status, "has_video": bool(r.video_path),
         "step_count": len(r.steps)}
    if with_steps: d["steps"] = [step_dict(s) for s in r.steps]
    return d
def var_dict(v):
...
app.mount("/", StaticFiles(directory="app/static", html=True), name="static")    scenarios = (demo_scenarios(body.source_text) if DEMO_MODE else await llm_scenarios(body.source_text))
    db = SessionLocal()
    try:
        created = []
        for sc in scenarios:
